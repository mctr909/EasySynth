namespace Synth {
	internal partial class Sampler {
		void WritePCM() {
			for (int i = 0; i < mChannel.InputSamples; i++) {
				/* ピッチ */
				double pitch;
				var tone = (int)((mVibIm * mVibDepth + mEgPitchValue + mChannel.Pitch) * 128);
				if (tone < 0) {
					tone *= -1;
				...
				/* エンベロープ更新 */
				switch (Id.State) {
				case SamplerState.PURGE:
					mEgAmpValue -= mEgAmpValue * 0.05;
					break;
				case SamplerState.PRESS:
					if (mEgAmpAttack) {
